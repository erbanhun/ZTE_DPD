...
figure()
plot(abs(y), abs(y_poly), '.')
hold on
plot(abs(y), abs(y_MP), '.')
title('AM-AM(功放反馈信号 - 模型输出的功放信号)')
legend('无记忆模型', 'Generalized-MP模型')

figure()
plot(angle(y ./ y_poly) * 180 / pi, '.')
hold on
plot(angle(y ./ y_MP) * 180 / pi, '.')
title('AM-PM(功放反馈信号 - 模型输出的功放信号)')
legend('无记忆模型', 'Generalized-MP模型')
